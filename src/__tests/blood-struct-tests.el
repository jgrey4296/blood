...
;;
(require 'buttercup)
(require 'extra-buttercup-matchers)
(require 'blood)
...
(describe "identifiers"
  ...
  ;;     (expect (blood--id-sym nil :profile t) :to-signal 'error)
  ;;     (expect (blood--id-sym iden) :to-signal 'error)
...
(describe "advice"
  ...
  ;;   (expect (blood-build-advice '(:targets #'message :advisors ((:around)))) :to-signal 'error)
  ;;   (expect (blood-build-advice '(:targets #'message :advisors ((:bad-target #'bloo)))) :to-signal 'error)
  ;;   )
)
...
(describe "paths"
  ...
  (it "should error on bad args"
    (expect (blood-build-paths '()) :to-signal 'error)
    )
  ;; (it "should build"
